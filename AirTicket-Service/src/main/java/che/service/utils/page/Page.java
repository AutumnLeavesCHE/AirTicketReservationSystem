 ...
 */
package che.service.utils.page;

import che.service.utils.cookie.CookieUtils;
import che.service.utils.json.JsonMapper;
...
public class Page<T> implements Serializable {
	...
	private List<T> list = new ArrayList<T>(); // 当前页数据对象
	
	private String orderBy = ""; // 标准查询有效， 实例： updatedate desc, name asc
	
	private int orderByRule = -1;//mongodb使用  1正序 -1 倒叙，一般按时间查询默认倒序
	...
	private Map<String, Object> otherData;	// 备用，外加数据集
	 ...
	 * @param pageNo 当前页码
	 * @param pageSize 分页大小
	 * @param count 数据条数，设置为“-1”表示不查询总数
	 */
...
//		}
//
//		sb.append("<li class=\"disabled controls\"><a href=\"javascript:\" style=\"cursor:default;\"");
//		sb.append(" title=\"点击数字，可填写页码和每页条数，按回车即可生效！\">当前 ");
...
//
//		sb.insert(0,"<ul class=\"pagination\" style=\"margin:8px 0 0 0;\">\n").append("</ul>\n");
//
...
//	}
	...
	/**
	 * 设置数据总数，设置为“-1”表示不查询总数，如果自定义Page要先进行setCount()，再进行setList()。
	 * @param count
	 */
	public void setCount(long count) {
		this.count = count;
		if (pageSize >= count){
			//pageNo = 1;// jaj解决查询超过最大页码的问题
		}
	}
...
//		return getLast();
		...
		int num = (int) (_count / _pageSize);
		int z = (_count % _pageSize) != 0 ? 1 : 0;
		return num + z;
	}
	...
	/**
	 * 设置本页数据对象列表，如果自定义Page要先进行setCount()，再进行setList()。
	 * @param list
	 */
	public Page<T> setList(List<T> list) {
		this.list = list;
		//initialize();// jaj解决查询超过最大页码的问题
		return this;
	}
	...
	public int getFirstResult(){
		...
		if (firstResult >= getCount()) {
			//firstResult = 0;// jaj解决查询超过最大页码的问题
		}
		return firstResult;
	}
	/**
	...
	public int getOrderByRule() {
		return orderByRule;
	}

	public void setOrderByRule(int orderByRule) {
...
//	}
	...
	public String toJsonString(){
		...
		map.put("totalCount", this.getCount());
		map.put("listData", this.getList());
